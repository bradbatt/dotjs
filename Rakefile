...
namespace :install do
  ...
  task :done do
    if system('curl -k https://localhost:3131 &> /dev/null')
      ...
      print 'then drop files like google.com.js'
      puts 'in ~/.js and enjoy hacking the web'
    else
  ...
  desc 'Create ~/.js'
  task :create_dir do
    unless File.directory? js_dir = File.join(ENV['HOME'], '.js')
      mkdir js_dir
      chmod 0755, js_dir
    end
  end
...
task :install_dir_writeable do
  unless File.writable?(DAEMON_INSTALL_DIR)
    abort %(Error: Can't write to #{DAEMON_INSTALL_DIR}.
